const rolService = require('../services/rol.service');
const { createSchema, updateSchema } = require('../validators/rol.validator');
const { BadRequestError } = require('../errors/httpErrors');

const getAll = async (req, res, next) => {
  try {
    const roles = await rolService.getAll();
    res.json({ status: 'ok', data: roles });
  } catch (err) {
    next(err);
  }
};

const getById = async (req, res, next) => {
  try {
    const rol = await rolService.getById(Number(req.params.id));
    res.json({ status: 'ok', data: rol });
  } catch (err) {
    next(err);
  }
};

const create = async (req, res, next) => {
  try {
    const { error, value } = createSchema.validate(req.body);
    if (error) throw new BadRequestError(error.details[0].message);

    const rol = await rolService.create(value);
    res.status(201).json({ status: 'ok', data: rol });
  } catch (err) {
    next(err);
  }
};

const update = async (req, res, next) => {
  try {
    const { error, value } = updateSchema.validate(req.body);
    if (error) throw new BadRequestError(error.details[0].message);

    const rol = await rolService.update(Number(req.params.id), value);
    res.json({ status: 'ok', data: rol });
  } catch (err) {
    next(err);
  }
};

const patchEstado = async (req, res, next) => {
  try {
    const { Estado } = req.body;
    if (Estado === undefined) throw new BadRequestError('El campo Estado es obligatorio');

    const rol = await rolService.toggleEstado(Number(req.params.id), Estado);
    res.json({ status: 'ok', data: rol });
  } catch (err) {
    next(err);
  }
};

module.exports = { getAll, getById, create, update, patchEstado };
