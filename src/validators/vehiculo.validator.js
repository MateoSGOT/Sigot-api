...
const createSchema = Joi.object({
  Placa:      Joi.string().max(10).required().messages({ 'any.required': 'La placa es obligatoria' }),
  VIN:        Joi.string().max(30).optional().allow('', null),
  Id_Cliente: Joi.number().integer().required().messages({ 'any.required': 'El cliente es obligatorio' }),
  Id_Marca:   Joi.number().integer().required().messages({ 'any.required': 'La marca es obligatoria' }),
  Modelo:     Joi.string().max(50).required().messages({ 'any.required': 'El modelo es obligatorio' }),
  Anio:       Joi.number().integer().min(1900).max(new Date().getFullYear() + 1).required().messages({
